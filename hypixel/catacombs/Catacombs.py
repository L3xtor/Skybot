...
class Catacombs(commands.Cog):
    ...
    def profileInfo(playername: str, PID):
        SkycryptProfileAPI = requests.get(
            f"https://sky.shiiyu.moe/api/v2/profile/{playername}"
        ).json()
        networth = numerize.numerize(
            SkycryptProfileAPI["profiles"][PID]["data"]["networth"]["networth"]
        )

        has_hype = ":x:"
        has_term = ":x:"

        for weapon in SkycryptProfileAPI["profiles"][PID]["data"]["items"]["weapons"][
            "weapons"
        ]:
    ...
    async def cata(self, ctx, playername: str, selectedprofile: str = None):
        ...
        PID, selectedprofile = returnProfileID(
            selectedprofile=selectedprofile, playername=playername
        )

        networth, has_hype, has_term = self.profileInfo(playername=playername, PID=PID)
        cata_level, secrets_found, secrets_per_run, _ = dungeonsInfo(
            playername=playername, selected_profile=selectedprofile
        )
        ...
        embed.add_field(
            name=f"Secrets found {secrets_found}",
            value=f"(Per Run: {secrets_per_run})",
            inline=False,
        )
