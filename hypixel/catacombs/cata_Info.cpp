...
extern "C" void get_skill_info(const char skill_type[15]) {
    ifstream file("./hypixel/utils/json/skill_levels.json");
...
extern "C" int find_skill_level(int skill_xp) {
    ...
    for (auto& [skill, xp_required] : cumulative_xp_required.items()) {
        // Check if the value matches the value you're looking for
        if (xp_required >= skill_xp) {
...
extern "C" double find_remaining_xp(double initial_exp, int skill_level) {
    double remaining_xp;

    // Iterate over each skill and cumulative xp required
    for (auto& [skill, cumulative_xp] : cumulative_xp_required.items()) {
        if (skill_level == stoi(skill)) {
            remaining_xp = initial_exp - (double)cumulative_xp;
            break;
        }
    }
...
extern "C" double calculate_xp_to_level_up(int skill_level) {
    for (auto& [skill, xp] : xp_required.items()) {
...
extern "C" SkillInfo getPlayerSkillInfo(double initial_exp) {
    // Initialize JSON with the chosen skill type

    // Initialize variables to return
    double remaining_xp, xp_required_to_level_up;
    int skill_level;
    ...
    for (size_t exp : cumulative_xp_required) {
        if (exp >= initial_exp) {
            ...
            remaining_xp = find_remaining_xp(initial_exp, skill_level);
