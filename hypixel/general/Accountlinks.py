...
from hypixel.utils.functions import player_data, connect_linkdb
...
class Accountslinks(commands.Cog):
    ...
    async def link(self, ctx: commands.Context, playername: str):
        ...
        interactionUser, interactionUserName, interactionUserID = (
            message_author.mention,
            message_author.name,
            message_author.id,
        )

        result = cursor.execute(
            f"SELECT is_linked FROM accountlinks WHERE discord_name = '{interactionUserName}'"
        ).fetchone()
        ...
        if result:
            ...
            try:
                ...
                if str(e) == "'id'":
                    embed.add_field(
                        name="No Minecraft Account found!",
                        value=f"User {playername} wasn't found in the Hypixel API",
                    )
                elif str(e) == "'socialMedia'":
                    embed.add_field(
                        name="No Discord Account found!",
                        value=f"User {playername} currently isn't linked with any Discord account",
                    )
            ...
            if linked_discord is not None and linked_discord == interactionUserName:
                ...
                embed.add_field(
                    name="Thank you!",
                    value=f"Thanks for veryfing *{interactionUser}*",
                    file=file,
                )

            else:
                embed.add_field(
                    name="Wrong Account linked!",
                    value=f"User '{playername}' has the discord name '{linked_discord}' linked",
                )
    ...
    async def whois(
        ...
        if result:
            discord_uuid, minecraft_uuid, discord_name, minecraft_name, is_linked = (
                result
            )
            ...
            embed.add_field(
                name="No Data Found",
                value=f"Account '{playername}' has no linked Discord",
            )
