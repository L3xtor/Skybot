...
class pfView(discord.ui.View):
    ...
    async def select_class(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        if interaction.user.id == self.party_creator_id:
            ...
            self.selected_classes[interaction.user.id] = self.chosenclass.lower()  # Store the creator's class
            await interaction.response.send_message(f"You've selected {self.chosenclass} as your class!", ephemeral=True)
            return

        # If the user is not the party creator and has already selected a class
        if interaction.user.id in self.selected_classes:
            await interaction.response.send_message("Already applied to this party.", ephemeral=True)
            return

        # For non-creator players, allow them to select a class
        self.chosenclass = select.values[0]
        self.selected_classes[interaction.user.id] = self.chosenclass.lower()  # Store their selected class

        role_names = ["🏹 Archer", "⚔️ Berserk", "🧙‍♂️ Mage", "🛡️ Tank", "⛑️ Healer"]
        class_roles = {role.lower(): f"<@&{discord.utils.get(interaction.guild.roles, name=role).id}>"
                       for role in role_names if discord.utils.get(interaction.guild.roles, name=role)}

        # Update remaining classes to exclude already selected ones
        remaining_classes = [cls for cls in class_roles if cls not in self.selected_classes.values()]

        # Construct the searching message
        searching_message = f"{interaction.user.mention} (playing {self.chosenclass}) is looking for: "
        ...
        async def button_callback(button_interaction: discord.Interaction):
            ...
            missing_class_view = MissingClassView(remaining_classes, self.selected_classes, self.party_creator_id)
...
class MissingClassView(discord.ui.View):
    def __init__(self, remaining_classes, selected_classes, party_creator_id):
        super().__init__()
        self.add_item(MissingClassDropdown(remaining_classes, selected_classes, party_creator_id))
...
class MissingClassDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        party_channel = discord.utils.get(interaction.guild.text_channels, name=f"lfp-{interaction.channel.name.split('-')[1]}")
