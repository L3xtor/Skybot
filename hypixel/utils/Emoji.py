...
class EmoteFunctions:
    def __init__(self) -> None:
        self.applicationurl = (
            "https://discord.com/api/v10/applications/1264605196466651249/emojis"
        )
        self.headers = {"Authorization": "Bot " + DISCORD_API_SECRET}
    ...
    def postdiscordemote(self, itemname: str):
        ...
        data = {"name": itemname, "image": dataurl}
    ...
    async def getemote(self, itemname):
        with open("hypixel/utils/json/emojis.json") as f:
            items = json.load(f)
...
class Emotes(commands.Cog):
    ...
    async def _get(self, ctx, itemname: str):
        markdown = await EmoteFunctions().getemote(itemname)
